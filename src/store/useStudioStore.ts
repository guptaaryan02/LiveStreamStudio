...
const markInstanceLive = (
  ...
  set((state) => ({
    debugStatus: '[9] SUCCESS - Stream is LIVE!',
...
export const useStudioStore = create<StudioState>()(
  ...
  startStream: async (instanceId) => {
    set({ debugStatus: `[1] startStream called for ${instanceId}` });
    ...
    set({ debugStatus: `[2] Instance found: ${inst.name}, playlist=${inst.playlistId}` });
    const playlist = playlists.find((p) => p.id === inst.playlistId);
    if (!playlist) {
      set({ debugStatus: `[FAIL] Playlist ${inst.playlistId} not found` });
      return;
    }

    set({ debugStatus: `[3] Playlist found: ${playlist.name} with ${playlist.videos.length} videos` });
    const cmd = generateFFmpegCommand(playlist, inst.platform, inst.rtmpUrl, inst.streamKey, inst.hardwareAcc, inst.targetBitrate || 4500, settings);

    try {
      const playlistTxt = generatePlaylistFileContent(playlist);
      set({ debugStatus: `[4] Playlist content: ${playlistTxt.substring(0, 80)}...` });
      ...
      const modeLog = createFFmpegLogLine(
        ...
        analysis.isCompatible
          ? `Fast Mode (stream copy) — ${clips.length} clip(s), compatibility ${analysis.score}%`
          : `Smart Compatibility Mode — ${clips.length} clip(s) normalised live to ${settings.outputResolution || '1080p'} @ ${hardwareAcc}` +
      ...
      set({ debugStatus: `[5] Generating FFmpeg args...` });
      ...
      set({ debugStatus: `[6] Args generated (${args.length} args). isTauri=${isTauriEnvironment()}` });

      if (isTauriEnvironment()) {
        set({ debugStatus: `[7] Calling Tauri startTauriFFmpegStream...` });
        const result = await startTauriFFmpegStream({
          streamId: inst.id,
          args: args,
          playlistContent: playlistTxt,
        });
        set({ debugStatus: `[8] Tauri result: success=${result.success} msg=${result.message || result.error}` });
        if (!result.success) {
        ...
        set({ debugStatus: `[7] Using web fetch fallback...` });
        ...
        set({ debugStatus: `[8] Web fetch completed` });
      }
    } catch (e) {
  ...
  logs: [],
  clearLogs: () => set({ logs: [] }),
  addLog: (log) => set((state) => ({ logs: [log, ...state.logs.slice(0, 499)] })),
  ...
  checkSchedules: () => {
    const { schedules, startStream, instances, createStreamInstance, profiles } = get();
    const now = new Date();
    ...
    const updatedSchedules = schedules.map(schedule => {
      if (!schedule.isActive || !schedule.autoStart) return schedule;
      ...
      if (schedule.repeatDays.includes(currentDay) && schedule.scheduledTime === currentTimeStr && !hasRunNow) {
        ...
        if (!isRunning) {
          ...
          const newInst = createStreamInstance(
            schedule.title,
            'default-playlist', // In a full app, schedule would also select a playlist
            prof ? prof.platform : 'youtube',
            prof ? prof.rtmpUrl : 'rtmp://a.rtmp.youtube.com/live2',
            prof ? prof.streamKey : 'dummy-key',
            'VideoToolbox'
          );
          ...
          return { ...schedule, lastRunDate: `${todayDateStr} ${currentTimeStr}` };
        }
      }
      return schedule;
    });
